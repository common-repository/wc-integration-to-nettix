...
class Nettimoto_Ad_Handler extends Ad_Handler {
	/**
	 * @return WP_Term
	 * @throws Exception
	 */
	public function get_main_category(): WP_Term {
		return $this->get_category( 'nettimoto' );
	}
	...
	private function create_short_description_content(
		...
		$ret = "";
		if ( $this->settings->get_service_bike_short_desc_use_list() == 'yes' ) {
			$ret .= "<ul>\n";

			if ( $this->settings->get_service_bike_short_desc_field_maker() == 'yes' && $maker ) {
				$ret .= "<li>";
				if ($this->settings->get_service_bike_short_desc_use_labels() == 'yes') {
					$ret .= "Merkki ja malli: ";
				}
				$ret .= $maker;
				if ( $this->settings->get_service_bike_short_desc_field_model() == 'yes' && $model ) {
					$ret .= " {$model}";
				}
				if ( $this->settings->get_service_bike_short_desc_field_model_type() == 'yes' && $model_type ) {
					$ret .= " {$model_type}";
				}
				$ret .= "</li>\n";
			}

			if ( $this->settings->get_service_bike_short_desc_field_year() == 'yes' && $year > 0 ) {
				$ret .= "<li>";
				if ($this->settings->get_service_bike_short_desc_use_labels() == 'yes') {
					$ret .= "Vuosimalli: ";
				}
			...
			if ( $this->settings->get_service_bike_short_desc_field_kms() == 'yes' && $kilometers >= 0 ) {
				$ret .= "<li>";
				if ($this->settings->get_service_bike_short_desc_use_labels() == 'yes') {
					$ret .= "Mittarilukema: ";
				}
			...
			if ( $this->settings->get_service_bike_short_desc_field_location() == 'yes' && $location > 0 ) {
				$ret .= "<li>";
				if ($this->settings->get_service_bike_short_desc_use_labels() == 'yes') {
					$ret .= "Sijainti: ";
				}
